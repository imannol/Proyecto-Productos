...
CREATE TABLE ventas(
pk_ventas INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
fecha_venta DATE NULL COMMENT 'Fecha de la venta',
sub_total FLOAT(8,2) NULL,
iva FLOAT(8,2) NULL,
...
FOREIGN KEY(fk_ventas) REFERENCES ventas(pk_ventas),
FOREIGN KEY(fk_productos) REFERENCES productos(pk_productos)
)


/* Consulta para obtener el, subtotal, iva y total
SELECT SUM(productos.precio_venta) AS 'Subtotal', SUM(productos.precio_venta)*0.16 AS 'IVA', SUM(productos.precio_venta)*1.16 AS 'Total', COUNT(productos.precio_venta) AS 'Num.productos Vendidos'
